...
ローカルにGoをインストール(go1.22.2の場合)

### intel Mac

```bash
wget https://go.dev/dl/go1.22.2.darwin-amd64.tar.gz
sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.22.2.darwin-amd64.tar.gz
```
...
sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.22.2.darwin-arm64.tar.gz
```
