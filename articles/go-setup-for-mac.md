...
ローカルにGoをインストール(go1.22.3の場合)

### intel Mac

```bash
wget https://go.dev/dl/go1.22.3.darwin-amd64.tar.gz
sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.22.3.darwin-amd64.tar.gz
```
...
sudo rm -rf /usr/local/go && sudo tar -C /usr/local -xzf go1.22.3.darwin-arm64.tar.gz
```
